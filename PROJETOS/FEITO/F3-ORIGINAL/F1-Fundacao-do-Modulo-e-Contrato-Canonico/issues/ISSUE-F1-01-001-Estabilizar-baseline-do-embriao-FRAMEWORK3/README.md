...
Esta issue materializa o recorte mais urgente do projeto. No estado atual do repositorio, o boot do backend quebrava quando `backend/app/main.py` carregava `app.models.__init__`, porque `backend/app/models/framework_models.py` declarava relacionamentos com `Mapped[...]` em um modelo `SQLModel` que nao era aceito pelo runtime atual. Os arquivos listados abaixo concentram o contrato que precisou ser consolidado neste passo do FRAMEWORK3. As precondicoes das tasks explicitam a ordem obrigatoria dentro da fase e da sprint.
