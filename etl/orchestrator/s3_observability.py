...
This module standardizes operational events for the Sprint 3 orchestration
path and provides actionable diagnostics correlated with backend telemetry.
"""
...
def build_s3_orchestrator_observability_event(
    ...
    Returns:
        S3OrchestratorObservabilityEvent: Structured event ready for logging.

    Raises:
        S3OrchestratorObservabilityError: If event name, correlation id,
            event message, severity, circuit state, or counters are invalid.
    """
    ...
    if circuit_state and circuit_state not in ALLOWED_CIRCUIT_STATES:
        raise S3OrchestratorObservabilityError(
            ...
            message=f"circuit_state de observabilidade invalido: {payload.circuit_state}",
    ...
    if route_position is not None and route_position < 1:
        raise S3OrchestratorObservabilityError(
            ...
            message="route_position de observabilidade do ORQ S3 deve ser >= 1",
            action="Informe posicao valida da rota no encadeamento de fallback.",
        )
    ...
    if total_routes is not None and total_routes < 1:
        raise S3OrchestratorObservabilityError(
            ...
            message="total_routes de observabilidade do ORQ S3 deve ser >= 1",
    ...
    if consecutive_failures is not None and consecutive_failures < 0:
        raise S3OrchestratorObservabilityError(
            ...
            message="consecutive_failures de observabilidade do ORQ S3 deve ser >= 0",
            action="Informe contador de falhas consecutivas nao negativo.",
        )
    ...
    if failure_threshold is not None and failure_threshold < 1:
        raise S3OrchestratorObservabilityError(
            ...
            message="failure_threshold de observabilidade do ORQ S3 deve ser >= 1",
            action="Informe limite de falhas valido para circuit breaker.",
        )
...
def log_s3_orchestrator_observability_event(event: S3OrchestratorObservabilityEvent) -> None:
    """Emit one ORQ Sprint 3 observability event to the operational logger.
