"""Unit tests for ORQ Sprint 4 scaffold, core, and service contracts."""
...
def test_s4_orchestrator_core_success_runs_main_flow() -> None:
    flow_input = S4OrchestratorCoreInput(
        ...
        rota_selecionada="agent_first_extract",
        decisao_telemetria_habilitada=True,
        custo_estimado_usd=0.3,
        custo_orcamento_usd=1.0,
        latencia_estimada_ms=1200,
        latencia_sla_ms=3000,
        telemetria_amostragem=0.5,
        correlation_id="orq-s4-core-001",
    )

    output = execute_s4_orchestrator_main_flow(flow_input).to_dict()

    assert output["contrato_versao"] == "orq.s4.core.v1"
    assert output["correlation_id"] == "orq-s4-core-001"
    assert output["status"] == "completed"
    assert output["rota_selecionada"] == "agent_first_extract"
    assert output["execucao"]["status"] == "succeeded"
    assert output["observabilidade"]["flow_started_event_id"].startswith("orqs4coreevt-")
    assert output["observabilidade"]["decision_recorded_event_id"].startswith("orqs4coreevt-")
    assert output["observabilidade"]["flow_completed_event_id"].startswith("orqs4coreevt-")


def test_s4_orchestrator_core_raises_actionable_error_for_failed_route() -> None:
    flow_input = S4OrchestratorCoreInput(
        source_id="SRC_TMJ_PDF_2025",
        source_kind="pdf",
        source_uri="file:///tmp/tmj_2025.pdf",
        ...
        latencia_sla_ms=3000,
        telemetria_amostragem=0.5,
        correlation_id="orq-s4-core-failure",
    )

    def fail_executor(route_name: str, route_context: dict[str, object]) -> dict[str, object]:
        return {
            "status": "failed",
            "decision_reason": f"{route_name}_failed",
            "latency_ms": 1200,
            "cost_usd": 0.2,
            "route_context": route_context,
        }

    with pytest.raises(S4OrchestratorCoreError) as exc:
        execute_s4_orchestrator_main_flow(flow_input, execute_route=fail_executor)

    error = exc.value
    assert error.code == "ORQ_S4_ROUTE_EXECUTION_FAILED"
    assert error.stage == "route_execution"
    assert (error.event_id or "").startswith("orqs4coreevt-")
...
def test_s4_orchestrator_service_success_returns_contract_and_observability() -> None:
    ...
    assert output["observabilidade"]["main_flow_started_event_id"].startswith("orqs4coreevt-")
    assert output["observabilidade"]["main_flow_decision_event_id"].startswith("orqs4coreevt-")
    assert output["observabilidade"]["main_flow_completed_event_id"].startswith("orqs4coreevt-")
