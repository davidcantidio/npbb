"""Telemetry helpers for CONF Sprint 1, Sprint 2, Sprint 3, and Sprint 4 flows.
...
COMPONENT_NAME_S4 = "motor_de_confianca_e_politica_de_decisao_s4"
...
ALLOWED_DECISIONS_S4 = {"auto_approve", "manual_review", "gap", "reject"}
ALLOWED_CRITICAL_VIOLATION_ROUTES_S3 = {"manual_review", "gap", "reject"}
ALLOWED_CRITICAL_VIOLATION_ROUTES_S4 = {"manual_review", "gap", "reject"}
...
class S4ConfidenceTelemetryContractError(ValueError):
    """Raised when CONF Sprint 4 telemetry contract input is invalid."""
...
@dataclass(frozen=True, slots=True)
class S4ConfidenceTelemetryInput:
    """Input contract for CONF Sprint 4 telemetry event creation."""
...
@dataclass(frozen=True, slots=True)
class S4ConfidenceTelemetryEvent:
    """Output contract for one structured CONF Sprint 4 telemetry event."""
...
def build_s4_confidence_telemetry_event(
    payload: S4ConfidenceTelemetryInput,
) -> S4ConfidenceTelemetryEvent:
    """Build a validated CONF Sprint 4 telemetry event.

    Args:
        payload: Input telemetry contract with operation context and severity.

    Returns:
        S4ConfidenceTelemetryEvent: Structured event ready for logging.

    Raises:
        S4ConfidenceTelemetryContractError: If event fields are invalid.
    """
    ...
    if not event_name:
        raise S4ConfidenceTelemetryContractError(
            code="INVALID_CONF_S4_TELEMETRY_EVENT_NAME",
            message="event_name de telemetria CONF S4 nao pode ser vazio",
            action="Defina o nome do evento operacional antes de emitir logs.",
        )
    if not correlation_id:
        raise S4ConfidenceTelemetryContractError(
            code="INVALID_CONF_S4_TELEMETRY_CORRELATION_ID",
            message="correlation_id de telemetria CONF S4 nao pode ser vazio",
            action="Propague correlation_id no fluxo antes de logar eventos.",
        )
    if not event_message:
        raise S4ConfidenceTelemetryContractError(
            code="INVALID_CONF_S4_TELEMETRY_EVENT_MESSAGE",
            message="event_message de telemetria CONF S4 nao pode ser vazio",
            action="Forneca descricao operacional para diagnostico do evento.",
        )
    if severity not in ALLOWED_SEVERITIES:
        raise S4ConfidenceTelemetryContractError(
            code="INVALID_CONF_S4_TELEMETRY_SEVERITY",
            message=f"severity de telemetria invalida: {payload.severity}",
            action="Use severidades aceitas: info, warning ou error.",
        )

    confidence_score = _validate_probability_s4(
        field_name="confidence_score",
        value=payload.confidence_score,
        error_code="INVALID_CONF_S4_TELEMETRY_CONFIDENCE_SCORE",
        error_action="Use confidence_score no intervalo de 0.0 a 1.0.",
    )

    decision = (payload.decision or "").strip().lower() or None
    if decision is not None and decision not in ALLOWED_DECISIONS_S4:
        raise S4ConfidenceTelemetryContractError(
            code="INVALID_CONF_S4_TELEMETRY_DECISION",
            message=f"decision de telemetria invalida: {payload.decision}",
            action="Use decision valida: auto_approve, manual_review, gap ou reject.",
        )

    manual_review_queue_size = _validate_non_negative_int_s4(
        field_name="manual_review_queue_size",
        value=payload.manual_review_queue_size,
        error_code="INVALID_CONF_S4_TELEMETRY_MANUAL_REVIEW_QUEUE_SIZE",
    )
    max_manual_review_queue = _validate_non_negative_int_s4(
        field_name="max_manual_review_queue",
        value=payload.max_manual_review_queue,
        error_code="INVALID_CONF_S4_TELEMETRY_MAX_MANUAL_REVIEW_QUEUE",
    )
    critical_fields_present_count = _validate_non_negative_int_s4(
        field_name="critical_fields_present_count",
        value=payload.critical_fields_present_count,
        error_code="INVALID_CONF_S4_TELEMETRY_CRITICAL_FIELDS_PRESENT_COUNT",
    )
    min_critical_fields_present = _validate_non_negative_int_s4(
        field_name="min_critical_fields_present",
        value=payload.min_critical_fields_present,
        error_code="INVALID_CONF_S4_TELEMETRY_MIN_CRITICAL_FIELDS_PRESENT",
    )
    feedback_samples_count = _validate_non_negative_int_s4(
        field_name="feedback_samples_count",
        value=payload.feedback_samples_count,
        error_code="INVALID_CONF_S4_TELEMETRY_FEEDBACK_SAMPLES_COUNT",
    )
    min_feedback_samples = _validate_non_negative_int_s4(
        field_name="min_feedback_samples",
        value=payload.min_feedback_samples,
        error_code="INVALID_CONF_S4_TELEMETRY_MIN_FEEDBACK_SAMPLES",
    )

    threshold_delta_applied = _validate_probability_s4(
        field_name="threshold_delta_applied",
        value=payload.threshold_delta_applied,
        error_code="INVALID_CONF_S4_TELEMETRY_THRESHOLD_DELTA_APPLIED",
        error_action="Use threshold_delta_applied no intervalo de 0.0 a 1.0.",
    )
    quality_drop_value = _validate_probability_s4(
        field_name="quality_drop_value",
        value=payload.quality_drop_value,
        error_code="INVALID_CONF_S4_TELEMETRY_QUALITY_DROP_VALUE",
    ...
    if (
        critical_violation_route is not None
        and critical_violation_route not in ALLOWED_CRITICAL_VIOLATION_ROUTES_S4
    ):
        raise S4ConfidenceTelemetryContractError(
            code="INVALID_CONF_S4_TELEMETRY_CRITICAL_VIOLATION_ROUTE",
            message=(
                "critical_violation_route de telemetria invalido: "
                f"{payload.critical_violation_route}"
            ),
    ...
    gap_escalation_triggered = _validate_optional_bool_s4(
        field_name="gap_escalation_triggered",
        value=payload.gap_escalation_triggered,
        error_code="INVALID_CONF_S4_TELEMETRY_GAP_ESCALATION_TRIGGERED",
    )
    critical_violation_triggered = _validate_optional_bool_s4(
        field_name="critical_violation_triggered",
        value=payload.critical_violation_triggered,
        error_code="INVALID_CONF_S4_TELEMETRY_CRITICAL_VIOLATION_TRIGGERED",
    )
    quality_drop_detected = _validate_optional_bool_s4(
        field_name="quality_drop_detected",
        value=payload.quality_drop_detected,
        error_code="INVALID_CONF_S4_TELEMETRY_QUALITY_DROP_DETECTED",
    )
    anomaly_detected = _validate_optional_bool_s4(
        field_name="anomaly_detected",
        value=payload.anomaly_detected,
        error_code="INVALID_CONF_S4_TELEMETRY_ANOMALY_DETECTED",
    )
    calibration_frozen = _validate_optional_bool_s4(
        field_name="calibration_frozen",
        value=payload.calibration_frozen,
        error_code="INVALID_CONF_S4_TELEMETRY_CALIBRATION_FROZEN",
    )
    tuned_thresholds = _normalize_tuned_thresholds_s4(payload.tuned_thresholds)

    return S4ConfidenceTelemetryEvent(
        telemetry_event_id=f"confs4evt-{uuid4().hex[:12]}",
        timestamp_utc=datetime.now(timezone.utc).isoformat(),
...
def emit_s4_confidence_telemetry_event(
    logger: logging.Logger,
    event: S4ConfidenceTelemetryEvent,
) -> None:
    """Emit one CONF Sprint 4 telemetry event to operational logger.
...
def build_s4_confidence_error_detail(
    *,
    code: str,
    message: str,
    action: str,
    ...
    """Build actionable CONF Sprint 4 error detail with telemetry references.
...
def _validate_non_negative_int_s4(
    *,
    field_name: str,
    value: int | None,
    error_code: str,
    ...
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise S4ConfidenceTelemetryContractError(
            code=error_code,
            message=f"{field_name} de telemetria CONF S4 deve ser inteiro >= 0",
            action=f"Propague {field_name} valido para rastreabilidade operacional.",
        )
    return value


def _validate_optional_bool_s4(
    *,
    field_name: str,
    value: bool | None,
    error_code: str,
    ...
    if not isinstance(value, bool):
        raise S4ConfidenceTelemetryContractError(
            code=error_code,
            message=f"{field_name} de telemetria CONF S4 deve ser booleano",
            action=f"Propague {field_name} como true/false para rastreabilidade operacional.",
        )
    return value


def _validate_probability_s4(
    *,
    field_name: str,
    value: float | None,
    error_code: str,
...
def _normalize_tuned_thresholds_s4(raw: dict[str, float] | None) -> dict[str, float] | None:
    if raw is None:
        return None
    if not isinstance(raw, dict):
        raise S4ConfidenceTelemetryContractError(
            code="INVALID_CONF_S4_TELEMETRY_TUNED_THRESHOLDS",
