...
def test_form_config_get_split_origin_usa_front_no_landing_e_backend_nos_demais_urls(
    ...
    with Session(engine) as session:
        ag1 = seed_agencia(session, "V3A", "v3a.com.br")
        tipo = seed_tipo(session)
        seed_user(session, "split-origin@example.com", "Senha123!", "npbb")
        evento = seed_evento(
            session,
            agencia_id=ag1.id,
            tipo_id=tipo.id,
    ...
    token = login_and_get_token(client, "split-origin@example.com", "Senha123!")
    resp = client.get(
        f"/evento/{evento_id}/form-config", headers={"Authorization": f"Bearer {token}"}
    )
    assert resp.status_code == 200
