---
doc_id: "ISSUE-F4-02-002-FORMALIZAR-FECHAMENTO-MANUAL-RECONCILED.md"
version: "1.0"
status: "todo"
owner: "PM"
...
# ISSUE-F4-02-002 - Formalizar fechamento manual_reconciled

## User Story

Como mantenedor do modulo de leads e dashboards, quero formalizar fechamento manual_reconciled para que a remediacao do vinculo canonico fique consistente e auditavel dentro de `EPIC-F4-02`.


## Feature de Origem

- **Feature**: Feature 3
- **Comportamento coberto**: fechamento manual rastreavel de manual_reconciled.
...
- Red: escrever ou ajustar testes focados nos contratos diretamente afetados por `ISSUE-F4-02-002` usando `cd backend && PYTHONPATH=.. SECRET_KEY=ci-secret-key TESTING=true python3 -m pytest -q tests/test_lead_event_service.py -k manual_reconciled` como comando alvo.
...
- Given os arquivos alvo de `ISSUE-F4-02-002`, When a issue for concluida, Then existe caminho explicito para registrar `manual_reconciled` sem perder rastreabilidade do fechamento manual.
