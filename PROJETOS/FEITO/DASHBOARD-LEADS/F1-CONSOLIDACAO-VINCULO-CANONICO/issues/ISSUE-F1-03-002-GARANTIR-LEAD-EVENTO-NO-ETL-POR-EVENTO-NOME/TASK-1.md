---
doc_id: "TASK-1.md"
issue_id: "ISSUE-F1-03-002-GARANTIR-LEAD-EVENTO-NO-ETL-POR-EVENTO-NOME"
task_id: "T1"
version: "1.0"
status: "todo"
owner: "PM"
...
# T1 - Garantir LeadEvento no ETL por evento_nome

## objetivo

Consolidar resolucao unica por `evento_nome` no ETL/import, sem criar vinculo em matches ambiguos ou ausentes.
...
- testes_a_escrever_primeiro:
  - cobrir o contrato principal de `ISSUE-F1-03-002` no modulo alvo
  - cobrir um caso de regressao ligado ao paradigma canonico `LeadEvento`
- comando_para_rodar:
  - `cd backend && PYTHONPATH=.. SECRET_KEY=ci-secret-key TESTING=true python3 -m pytest -q tests/test_lead_silver_mapping.py -k evento`
- criterio_red:
...
- `cd backend && PYTHONPATH=.. SECRET_KEY=ci-secret-key TESTING=true python3 -m pytest -q tests/test_lead_silver_mapping.py -k evento`
- `rg -n "lead_evento|LeadEvento|LeadEventoSourceKind|AtivacaoLead|evento_nome|lead_batch|manual_reconciled|dashboard" backend/app/modules/leads_publicidade/application/etl_import/persistence.py backend/app/services/lead_event_service.py backend/tests/test_lead_silver_mapping.py`
...
- match unico por `evento_nome` cria `LeadEvento`
- match ambiguo ou ausente nao cria vinculo silencioso
