---
doc_id: "TASK-1.md"
issue_id: "ISSUE-F4-02-001-GERAR-RELATORIO-DE-RECONCILIACAO-MISSING-E-AMBIGUOUS"
task_id: "T1"
version: "1.0"
status: "todo"
owner: "PM"
...
# T1 - Gerar relatorio de reconciliacao missing e ambiguous

## objetivo

Emitir artefato rastreavel com os casos `missing` e `ambiguous` encontrados pelo backfill.
...
- testes_a_escrever_primeiro:
  - cobrir o contrato principal de `ISSUE-F4-02-001` no modulo alvo
  - cobrir um caso de regressao ligado ao paradigma canonico `LeadEvento`
- comando_para_rodar:
  - `cd backend && PYTHONPATH=.. SECRET_KEY=ci-secret-key TESTING=true python3 -m pytest -q tests/test_lead_event_service.py -k reconcile`
- criterio_red:
...
- `cd backend && PYTHONPATH=.. SECRET_KEY=ci-secret-key TESTING=true python3 -m pytest -q tests/test_lead_event_service.py -k reconcile`
- `rg -n "lead_evento|LeadEvento|LeadEventoSourceKind|AtivacaoLead|evento_nome|lead_batch|manual_reconciled|dashboard" backend/app/services/lead_event_service.py backend/tests/test_lead_event_service.py artifacts/lead_evento_reconciliation/`
...
- casos `missing` entram no relatorio
- casos `ambiguous` entram no relatorio sem criar vinculo
