"""Observability contracts for Sprint 2 batch reception flow.

This module standardizes frontend-side operational events for Sprint 2 and
...
COMPONENT_NAME = "frontend_ingestao_inteligente_s2"
ALLOWED_SEVERITIES = {"info", "warning", "error"}


class S2ObservabilityError(ValueError):
    """Raised when Sprint 2 observability contract is invalid."""
...
class S2ObservabilityInput:
    """Input contract used to create one Sprint 2 observability event."""
...
@dataclass(frozen=True, slots=True)
class S2ObservabilityEvent:
    """Output contract for one Sprint 2 frontend observability event."""
...
def build_s2_observability_event(payload: S2ObservabilityInput) -> S2ObservabilityEvent:
    """Build validated observability event for Sprint 2 frontend flow.

    Args:
        payload: Input observability data from one stage of Sprint 2 flow.

    Returns:
        S2ObservabilityEvent: Structured event ready to be logged.

    Raises:
        S2ObservabilityError: If required observability fields are invalid.
    """
    ...
    if not event_name:
        raise S2ObservabilityError(
            code="INVALID_S2_OBSERVABILITY_EVENT_NAME",
    ...
    if severity not in ALLOWED_SEVERITIES:
        raise S2ObservabilityError(
            code="INVALID_S2_OBSERVABILITY_SEVERITY",
...
def log_s2_observability_event(event: S2ObservabilityEvent) -> None:
    """Emit observability event to Sprint 2 frontend operational logger.
...
def build_s2_actionable_error(
    *,
    ...
    """Build standardized actionable error dictionary for Sprint 2 flow.
    ...
    Returns:
        dict[str, Any]: Error payload aligned with Sprint 2 diagnostics contract.
    """
