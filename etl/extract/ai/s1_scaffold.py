"""Sprint 1 scaffold contracts for AI extraction on bounded formats.

This module defines the stable input/output contract used by XIA Sprint 1 to
prepare AI extraction for digital PDF and DOCX sources with actionable
diagnostics.
"""
...
logger = logging.getLogger("npbb.etl.extract.ai.s1")

CONTRACT_VERSION = "xia.s1.v1"
BACKEND_XIA_S1_PREPARE_ENDPOINT = "/internal/etl/extract/ai/s1/prepare"
SOURCE_ID_RE = re.compile(r"^[A-Z0-9][A-Z0-9_]{2,159}$")

ALLOWED_SOURCE_KINDS = ("pdf", "docx")
ALLOWED_MODEL_PROVIDERS = ("openai", "azure_openai", "anthropic", "local")
ALLOWED_CHUNK_STRATEGIES = ("section", "page", "paragraph")
ALLOWED_PDF_PROFILE_HINTS = ("pdf_digital", "pdf_text_table", "pdf_textual")
ALLOWED_DOCX_PROFILE_HINTS = ("docx_textual", "docx_sections", "docx_template")
MIN_MAX_TOKENS = 128
MAX_MAX_TOKENS = 8192


class S1AIExtractScaffoldError(ValueError):
    """Raised when XIA S1 scaffold contract input is invalid."""
...
class S1AIExtractScaffoldRequest:
    """Input contract for XIA Sprint 1 scaffold validation."""
    ...
    def to_backend_payload(self) -> dict[str, Any]:
        """Serialize request in backend-compatible XIA Sprint 1 contract."""
...
@dataclass(frozen=True, slots=True)
class S1AIExtractScaffoldResponse:
    """Output contract returned when XIA Sprint 1 scaffold is valid."""
...
def build_s1_ai_extract_scaffold_contract(
    request: S1AIExtractScaffoldRequest,
) -> S1AIExtractScaffoldResponse:
    """Build XIA Sprint 1 scaffold contract for AI extraction preparation.
    ...
    Returns:
        S1AIExtractScaffoldResponse: Stable scaffold output with extraction
            plan and integration points.

    Raises:
        S1AIExtractScaffoldError: If one or more XIA Sprint 1 input rules fail.
    """

    correlation_id = request.correlation_id or f"xia-s1-{uuid4().hex[:12]}"
    logger.info(
        "etl_extract_ai_s1_scaffold_input_received",
        extra={
    ...
    extraction_plan = {
        "modo_extracao": "ai_delimitada",
        "source_kind": source_kind,
        "document_profile_hint": document_profile_hint,
    ...
    response = S1AIExtractScaffoldResponse(
        ...
        pontos_integracao={
            "xia_s1_prepare_endpoint": BACKEND_XIA_S1_PREPARE_ENDPOINT,
            "extract_ai_service_module": "app.services.etl_extract_ai_service.execute_s1_extract_ai_service",
            "orchestrator_route_module": "app.services.etl_orchestrator_service.execute_s4_orchestrator_service",
            "pdf_assisted_module": "etl.extract.extract_pdf_assisted.extract_pdf_assisted",
        },
    )
    logger.info(
        "etl_extract_ai_s1_scaffold_ready",
        extra={
            "correlation_id": correlation_id,
            "source_id": source_id,
            "source_kind": source_kind,
...
def _validate_s1_input(
    *,
    request: S1AIExtractScaffoldRequest,
) -> tuple[str, str, str, str | None, str, str, str, int, float]:
    ...
    if source_kind not in ALLOWED_SOURCE_KINDS:
        _raise_contract_error(
            ...
            action="Use source_kind suportado para sprint 1: pdf ou docx.",
        )
    ...
    if profile_hint:
        allowed_hints = ALLOWED_PDF_PROFILE_HINTS if source_kind == "pdf" else ALLOWED_DOCX_PROFILE_HINTS
        if profile_hint not in allowed_hints:
            _raise_contract_error(
                code="INVALID_DOCUMENT_PROFILE_HINT",
                message=f"document_profile_hint invalido para {source_kind}: {request.document_profile_hint}",
                action="Use profile hint compativel com o formato da fonte.",
            )
    ...
    if chunk_strategy not in ALLOWED_CHUNK_STRATEGIES:
        _raise_contract_error(
            ...
            action="Use chunk_strategy suportada: section, page ou paragraph.",
        )
...
def _raise_contract_error(*, code: str, message: str, action: str) -> None:
    logger.warning(
        "etl_extract_ai_s1_scaffold_contract_error",
        extra={
            "error_code": code,
    ...
    raise S1AIExtractScaffoldError(code=code, message=message, action=action)
