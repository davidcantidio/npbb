...
case "${DEPLOY_MODE:-ip}" in
  ...
  domain)
    ...
    : "${API_DOMAIN:?API_DOMAIN is required when DEPLOY_MODE=domain}"
    render '${APP_DOMAIN}' "app.conf.template" "app.conf"
    render '${API_DOMAIN}' "api.conf.template" "api.conf"
    if [ "${ENABLE_TLS:-false}" = "true" ]; then
      ...
      render '${API_DOMAIN} ${TLS_CERT_PATH} ${TLS_KEY_PATH}' "api.tls.conf.template" "api-tls.conf"
    fi
    ;;
  *)
