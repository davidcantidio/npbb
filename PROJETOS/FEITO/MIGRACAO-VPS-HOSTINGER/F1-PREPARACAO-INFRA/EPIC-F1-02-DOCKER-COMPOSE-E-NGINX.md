...
**Plano TDD**
1. `Red`: endurecer `scripts/deploy_vps.sh --check-only` e `scripts/check_production_stack_contracts.sh` para falhar quando a stack nao expuser exatamente `postgres`, `api`, `web`, `edge` e `backup`, ou quando faltarem `restart: unless-stopped` e healthchecks minimos.
2. `Green`: revisar `Infra/production/docker-compose.yml`, `frontend/Dockerfile` e `Infra/production/backup/Dockerfile` para manter os cinco servicos atuais, com healthchecks em `postgres`, `api`, `web`, `edge` e contrato operacional verificavel para `backup`.
...
- Given a stack subida na VPS, When `docker compose ps` e `docker inspect` sao inspecionados, Then `postgres`, `api`, `web`, `edge` e `backup` aparecem saudaveis/operacionais conforme o contrato definido para a fase.
...
3. `Refactor`: documentar em `docs/DEPLOY_HOSTINGER_VPS.md` a equivalencia conceitual `edge = Nginx`, evitando ambiguidade entre PRD e scaffold atual.
...
2. `Green`: fazer `scripts/smoke_vps.sh` verificar health dos containers, endpoints HTTP obrigatorios, smoke de login invalido e, opcionalmente, smoke autenticado quando `SMOKE_LOGIN_*` estiver preenchido.
