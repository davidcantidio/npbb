"""Validation contracts for XIA Sprint 2 extraction on PPTX/XLSX/CSV.

This module centralizes XIA Sprint 2 input/output validation used by tests and
...
logger = logging.getLogger("npbb.etl.extract.ai.s2.validation")

XIA_S2_VALIDATION_VERSION = "xia.s2.validation.v1"
ALLOWED_FLOW_STATUSES = {"completed"}
ALLOWED_EXEC_STATUSES = {"succeeded", "completed", "success"}


class S2AIExtractValidationError(ValueError):
    """Raised when XIA Sprint 2 validation contract is violated."""
...
class S2AIExtractValidationInput:
    """Input contract consumed by XIA Sprint 2 validation checks."""
    ...
    def to_core_input(self, *, correlation_id: str | None = None) -> S2AIExtractCoreInput:
        """Convert validated data to `S2AIExtractCoreInput` contract."""
    ...
    def to_scaffold_request(
        ...
        *,
        ...
        """Convert validated data to `S2AIExtractScaffoldRequest` contract."""
...
@dataclass(frozen=True, slots=True)
class S2AIExtractValidationResult:
    """Output contract returned by XIA Sprint 2 input validation."""
...
@dataclass(frozen=True, slots=True)
class S2AIExtractFlowOutputValidationResult:
    """Output contract returned by XIA Sprint 2 flow-output validation."""
...
def validate_s2_extract_ai_input_contract(
    payload: S2AIExtractValidationInput,
) -> S2AIExtractValidationResult:
    """Validate XIA Sprint 2 input contract before running the main flow.

    Args:
        payload: Input contract with source metadata and AI extraction params.

    Returns:
        S2AIExtractValidationResult: Validation metadata and checks summary.

    Raises:
        S2AIExtractValidationError: If mandatory contract rules are violated.
    """

    correlation_id = payload.correlation_id or f"xia-s2-val-{uuid4().hex[:12]}"
    started_event = _emit_observability_event(
        event_name="etl_extract_ai_s2_validation_started",
        correlation_id=correlation_id,
        event_message="Validacao de entrada do XIA S2 iniciada",
        severity="info",
        ...
        tabular_layout_hint=payload.tabular_layout_hint,
    ...
    completed_event = _emit_observability_event(
        event_name="etl_extract_ai_s2_validation_completed",
        correlation_id=correlation_id,
        event_message="Validacao de entrada do XIA S2 concluida com sucesso",
        severity="info",
    ...
    logger.info(
        "etl_extract_ai_s2_validation_result",
        extra={
            "validation_id": validation_id,
            "correlation_id": correlation_id,
            "status": "valid",
            "checks": checks,
            "route_preview": "ai_delimitada_s2",
        },
    )
    return S2AIExtractValidationResult(
        validation_version=XIA_S2_VALIDATION_VERSION,
        validation_id=validation_id,
        correlation_id=correlation_id,
        status="valid",
        checks=tuple(checks),
...
def validate_s2_extract_ai_flow_output_contract(
    flow_output: dict[str, Any],
    *,
    correlation_id: str,
) -> S2AIExtractFlowOutputValidationResult:
    """Validate XIA Sprint 2 flow output contract.

    Args:
        flow_output: Output dictionary produced by XIA S2 core or service flow.
        correlation_id: Correlation identifier propagated by caller.

    Returns:
        S2AIExtractFlowOutputValidationResult: Summary of checked fields.

    Raises:
        S2AIExtractValidationError: If output contract is incomplete or
            inconsistent with XIA Sprint 2 specifications.
    """
    ...
    if missing_fields:
        _raise_validation_error(
            ...
            message=f"Saida do fluxo XIA S2 incompleta: faltam campos {missing_fields}",
    ...
    if status not in ALLOWED_FLOW_STATUSES:
        _raise_validation_error(
            ...
            message=f"Status final inesperado no fluxo XIA S2: {flow_output.get('status')}",
            action="Garanta status 'completed' para saidas validas do XIA S2.",
            correlation_id=correlation_id,
            stage="validation_output",
    ...
    logger.info(
        "etl_extract_ai_s2_flow_output_validated",
        extra={
            "correlation_id": correlation_id,
            "status": "valid",
            "layer": layer,
    ...
    return S2AIExtractFlowOutputValidationResult(
        correlation_id=correlation_id,
        status="valid",
        layer=layer,
        checked_fields=required_fields,
...
def _resolve_output_layer(
    *,
    ...
    if contract_version == "xia.s2.core.v1":
        return (
            "core",
            {
                "flow_started_event_id": "xias2coreevt-",
                "flow_completed_event_id": "xias2coreevt-",
            },
        )
    if contract_version == "xia.s2.service.v1":
        return (
            "service",
            {
                "flow_started_event_id": "xias2evt-",
                "plan_ready_event_id": "xias2evt-",
                "flow_completed_event_id": "xias2evt-",
                "main_flow_started_event_id": "xias2coreevt-",
                "main_flow_completed_event_id": "xias2coreevt-",
            },
        )
    raise S2AIExtractValidationError(
        code="UNSUPPORTED_OUTPUT_CONTRACT_VERSION",
        message=f"versao de contrato nao suportada para validacao: {contract_version}",
        action="Use contratos xia.s2.core.v1 ou xia.s2.service.v1.",
        correlation_id=correlation_id,
        stage="validation_output",
    )
...
def _raise_validation_error(
    *,
    ...
    failed_event = _emit_observability_event(
        event_name="etl_extract_ai_s2_validation_failed",
        correlation_id=correlation_id,
    ...
    logger.warning(
        "etl_extract_ai_s2_validation_error",
        extra={
            "correlation_id": correlation_id,
            "error_code": code,
...
def _emit_observability_event(
    *,
    ...
    log_s2_extract_ai_observability_event(event)
    return event
