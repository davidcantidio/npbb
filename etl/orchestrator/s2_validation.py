"""Validation contracts for ORQ Sprint 2 deterministic-first orchestration.

This module centralizes ORQ Sprint 2 input/output validation used by tests and
...
from .s2_core import S2OrchestratorCoreInput
...
logger = logging.getLogger("npbb.etl.orchestrator.s2.validation")

S2_ORCHESTRATOR_VALIDATION_VERSION = "orq.s2.validation.v1"


class S2OrchestratorValidationError(ValueError):
    """Raised when ORQ Sprint 2 validation contract is violated."""
...
class S2OrchestratorValidationInput:
    """Input contract consumed by ORQ Sprint 2 validation checks."""
    ...
    def to_core_input(self, *, correlation_id: str | None = None) -> S2OrchestratorCoreInput:
        """Convert validated data to `S2OrchestratorCoreInput` contract."""
    ...
    def to_scaffold_request(
        ...
        *,
        ...
        """Convert validated data to `S2OrchestratorScaffoldRequest` contract."""
...
@dataclass(frozen=True, slots=True)
class S2OrchestratorValidationResult:
    """Output contract returned by ORQ Sprint 2 input validation."""
...
@dataclass(frozen=True, slots=True)
class S2OrchestratorFlowOutputValidationResult:
    """Output contract returned by ORQ Sprint 2 flow-output validation."""
...
def validate_s2_orchestrator_input_contract(
    payload: S2OrchestratorValidationInput,
) -> S2OrchestratorValidationResult:
    """Validate ORQ Sprint 2 input contract before running the main flow.

    Args:
        payload: Input contract with source metadata, selected route, and
            runtime fallback policy.

    Returns:
        S2OrchestratorValidationResult: Validation metadata and checks summary.

    Raises:
        S2OrchestratorValidationError: If mandatory contract rules are violated.
    """

    correlation_id = payload.correlation_id or f"orq-s2-val-{uuid4().hex[:12]}"
    started_event = _emit_observability_event(
        event_name="etl_orchestrator_s2_validation_started",
        correlation_id=correlation_id,
        event_message="Validacao de entrada do ORQ S2 iniciada",
        severity="info",
    ...
    if not payload.source_kind.strip():
        _raise_validation_error(
            ...
            action="Informe tipo da fonte para selecionar rotas deterministicas.",
            correlation_id=correlation_id,
            stage="validation_input",
        )
    ...
    if not payload.rota_selecionada.strip():
        _raise_validation_error(
            ...
            action="Informe rota inicial para o fluxo deterministic-first do ORQ S2.",
            correlation_id=correlation_id,
            stage="validation_input",
        )
    ...
    try:
        ...
        _raise_validation_error(
            ...
            context={"source_kind": payload.source_kind, "rota_selecionada": payload.rota_selecionada},
        )
        raise AssertionError("unreachable")
    checks.append("scaffold_contract")

    completed_event = _emit_observability_event(
        event_name="etl_orchestrator_s2_validation_completed",
        correlation_id=correlation_id,
        event_message="Validacao de entrada do ORQ S2 concluida com sucesso",
        severity="info",
    ...
    logger.info(
        "etl_orchestrator_s2_validation_result",
        extra={
    ...
    return S2OrchestratorValidationResult(
        validation_version=S2_ORCHESTRATOR_VALIDATION_VERSION,
        validation_id=validation_id,
        correlation_id=correlation_id,
        status="valid",
        checks=tuple(checks),
...
def validate_s2_orchestrator_flow_output_contract(
    flow_output: dict[str, Any],
    *,
    correlation_id: str,
) -> S2OrchestratorFlowOutputValidationResult:
    """Validate ORQ Sprint 2 flow output contract.

    Args:
        flow_output: Output dictionary produced by ORQ S2 core or service flow.
        correlation_id: Correlation identifier propagated by caller.

    Returns:
        S2OrchestratorFlowOutputValidationResult: Summary of checked fields.

    Raises:
        S2OrchestratorValidationError: If output contract is incomplete or
            inconsistent with ORQ Sprint 2 specifications.
    """
    ...
    if missing_fields:
        _raise_validation_error(
            ...
            message=f"Saida do fluxo ORQ S2 incompleta: faltam campos {missing_fields}",
    ...
    if layer == "core" and "execucao" not in flow_output:
        _raise_validation_error(
            ...
            message="Saida core do ORQ S2 deve incluir campo execucao",
    ...
    if str(flow_output.get("status", "")).lower() != "completed":
        _raise_validation_error(
            ...
            message=f"Status final inesperado no fluxo ORQ S2: {flow_output.get('status')}",
            action="Garanta status 'completed' para saidas validas do ORQ S2.",
            correlation_id=correlation_id,
            stage="validation_output",
    ...
    if layer == "core":
        ...
        if final_route and final_route != top_route:
            _raise_validation_error(
                ...
                action="Garanta consistencia da rota final no contrato core do ORQ S2.",
                correlation_id=correlation_id,
                stage="validation_output",
    ...
    logger.info(
        "etl_orchestrator_s2_flow_output_validated",
        extra={
            "correlation_id": correlation_id,
            "status": "valid",
            "layer": layer,
    ...
    return S2OrchestratorFlowOutputValidationResult(
        correlation_id=correlation_id,
        status="valid",
        layer=layer,
        checked_fields=required_fields,
...
def _resolve_output_layer(
    *,
    ...
    if contract_version == "orq.s2.core.v1":
        return (
            "core",
            {
                "flow_started_event_id": "orqs2coreevt-",
                "route_resolved_event_id": "orqs2coreevt-",
                "flow_completed_event_id": "orqs2coreevt-",
            },
        )
    if contract_version == "orq.s2.service.v1":
        return (
            "service",
            {
                "scaffold_started_event_id": "orqs2evt-",
                "route_decision_event_id": "orqs2evt-",
                "scaffold_completed_event_id": "orqs2evt-",
                "main_flow_started_event_id": "orqs2coreevt-",
                "main_flow_completed_event_id": "orqs2coreevt-",
            },
        )
    raise S2OrchestratorValidationError(
        code="UNSUPPORTED_OUTPUT_CONTRACT_VERSION",
        message=f"versao de contrato nao suportada para validacao: {contract_version}",
        action="Use contratos orq.s2.core.v1 ou orq.s2.service.v1.",
        correlation_id=correlation_id,
        stage="validation_output",
    )
...
def _raise_validation_error(
    *,
    ...
    failed_event = _emit_observability_event(
        event_name="etl_orchestrator_s2_validation_failed",
        correlation_id=correlation_id,
    ...
    logger.warning(
        "etl_orchestrator_s2_validation_error",
        extra={
            "correlation_id": correlation_id,
            "error_code": code,
