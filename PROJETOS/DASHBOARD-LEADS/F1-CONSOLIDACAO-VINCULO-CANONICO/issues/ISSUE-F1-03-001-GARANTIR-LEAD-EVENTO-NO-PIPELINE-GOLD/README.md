...
# ISSUE-F1-03-001 - Garantir LeadEvento no pipeline Gold

## User Story

Como mantenedor do modulo de leads e dashboards, quero garantir leadevento no pipeline gold para que a remediacao do vinculo canonico fique consistente e auditavel dentro de `EPIC-F1-03`.
...
- Red: escrever ou ajustar testes focados nos contratos diretamente afetados por `ISSUE-F1-03-001` usando `cd backend && PYTHONPATH=.. SECRET_KEY=ci-secret-key TESTING=true python3 -m pytest -q tests/test_lead_gold_pipeline.py` como comando alvo.
...
- Given os arquivos alvo de `ISSUE-F1-03-001`, When a issue for concluida, Then leads promovidos a Gold asseguram `LeadEvento` quando o lote conhece `evento_id`.
