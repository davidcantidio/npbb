"""Main flow for XIA Sprint 3 AI extraction on scanned documents and images.
...
logger = logging.getLogger("npbb.etl.extract.ai.s3.core")

CONTRACT_VERSION = "xia.s3.core.v1"
XIA_S3_SUCCESS_STATUSES = {"succeeded", "completed", "success"}
XIA_S3_FAILED_STATUSES = {"failed", "fatal_error", "timeout", "transient_error", "retryable_failure"}
XIA_S3_ALLOWED_EXEC_STATUSES = XIA_S3_SUCCESS_STATUSES | XIA_S3_FAILED_STATUSES


class S3AIExtractCoreError(RuntimeError):
    """Raised when XIA Sprint 3 core flow fails."""
...
class S3AIExtractCoreInput:
    """Input contract consumed by XIA Sprint 3 core flow."""
    ...
    def to_scaffold_request(self, *, correlation_id: str) -> S3AIExtractScaffoldRequest:
        """Build scaffold request using XIA Sprint 3 stable fields."""
...
@dataclass(frozen=True, slots=True)
class S3AIExtractCoreOutput:
    """Output contract returned by XIA Sprint 3 core flow."""
...
def execute_s3_ai_extract_main_flow(
    flow_input: S3AIExtractCoreInput,
    *,
    execute_extraction: Callable[[dict[str, Any]], dict[str, Any]] | None = None,
) -> S3AIExtractCoreOutput:
    """Execute XIA Sprint 3 main flow with stable contracts and diagnostics.
    ...
    Returns:
        S3AIExtractCoreOutput: Stable output with extraction plan,
            execution diagnostics, and observability identifiers.

    Raises:
        S3AIExtractCoreError: If scaffold validation fails, extraction response
            is invalid, or extraction execution fails.
    """

    correlation_id = flow_input.correlation_id or f"xia-s3-{uuid4().hex[:12]}"
    extractor = execute_extraction or _default_extractor
    started_event_id = _new_event_id()
    logger.info(
        "etl_extract_ai_s3_main_flow_started",
        extra={
            "correlation_id": correlation_id,
    ...
    try:
        ...
        logger.info(
            "etl_extract_ai_s3_main_flow_completed",
            extra={
                "correlation_id": correlation_id,
        ...
        if status not in XIA_S3_SUCCESS_STATUSES:
            raise S3AIExtractCoreError(
                code="XIA_S3_EXTRACTION_FAILED",
                message=f"Execucao de extracao XIA S3 falhou com status '{status}'",
                action=(
                    "Revisar logs de execucao da extracao IA para PDF scan/JPG/PNG "
                    "e validar preprocessamento/chunk strategy."
                ),
                correlation_id=correlation_id,
                stage="extraction",
                event_id=completed_event_id,
        ...
        pontos_integracao["extract_ai_core_module"] = (
            "etl.extract.ai.s3_core.execute_s3_ai_extract_main_flow"
        )
        return S3AIExtractCoreOutput(
        ...
        logger.warning(
            "etl_extract_ai_s3_main_flow_scaffold_error",
            extra={
                "correlation_id": correlation_id,
    ...
    except S3AIExtractCoreError:
        raise
    except Exception as exc:
        failed_event_id = _new_event_id()
        logger.error(
            "etl_extract_ai_s3_main_flow_unexpected_error",
            extra={
                "correlation_id": correlation_id,
                "event_id": failed_event_id,
        ...
        raise S3AIExtractCoreError(
            code="XIA_S3_MAIN_FLOW_FAILED",
            message=f"Falha ao executar fluxo principal XIA S3: {type(exc).__name__}",
            action="Revisar logs operacionais e validar contrato de entrada/executor XIA S3.",
            correlation_id=correlation_id,
            stage="main_flow",
            event_id=failed_event_id,
        ) from exc
...
def _normalize_execution_response(
    *,
    ...
    if status not in XIA_S3_ALLOWED_EXEC_STATUSES:
        raise S3AIExtractCoreError(
...
def _new_event_id() -> str:
    return f"xias3coreevt-{uuid4().hex[:12]}"
