"""Main flow for CONT Sprint 2 strict schema/domain contract validation.

This module executes the Sprint 2 core path:
1) validate input and resolve Sprint 2 scaffold profile;
2) execute one strict schema/domain validation pass (dry-run by default);
...
CONTRACT_VERSION = "cont.s2.core.v1"
CONT_S2_SUCCESS_STATUSES = {"succeeded", "completed", "success"}
CONT_S2_FAILED_STATUSES = {"failed", "fatal_error", "timeout", "transient_error", "retryable_failure"}
CONT_S2_ALLOWED_EXEC_STATUSES = CONT_S2_SUCCESS_STATUSES | CONT_S2_FAILED_STATUSES


class S2CanonicalContractCoreError(RuntimeError):
    """Raised when CONT Sprint 2 core flow fails."""
...
class S2CanonicalContractCoreInput:
    """Input contract consumed by CONT Sprint 2 core flow."""
    ...
    def to_scaffold_request(self, *, correlation_id: str) -> S2CanonicalContractScaffoldRequest:
        """Build scaffold request using CONT Sprint 2 stable fields."""
...
@dataclass(frozen=True, slots=True)
class S2CanonicalContractCoreOutput:
    """Output contract returned by CONT Sprint 2 core flow."""
...
def execute_s2_contract_validation_main_flow(
    flow_input: S2CanonicalContractCoreInput,
    *,
    execute_validation: Callable[[dict[str, Any]], dict[str, Any]] | None = None,
) -> S2CanonicalContractCoreOutput:
    """Execute CONT Sprint 2 main flow with stable contracts and diagnostics.

    Args:
        flow_input: Input contract with canonical metadata, required schema
            fields, and domain constraints.
    ...
    Returns:
        S2CanonicalContractCoreOutput: Stable output with validation profile,
            execution diagnostics, and observability identifiers.

    Raises:
        S2CanonicalContractCoreError: If scaffold validation fails, validation
            response is invalid, or validation execution fails.
    """

    correlation_id = flow_input.correlation_id or f"cont-s2-{uuid4().hex[:12]}"
    validator = execute_validation or _default_validator
    started_event_id = _new_event_id()
    logger.info(
        "contract_validation_s2_main_flow_started",
        extra={
            "correlation_id": correlation_id,
    ...
    try:
        ...
        validation_context = {
            ...
            "required_fields_count": scaffold.validation_profile.get("required_fields_count", 0),
            "domain_rules_count": scaffold.validation_profile.get("domain_rules_count", 0),
        }
        ...
        logger.info(
            "contract_validation_s2_main_flow_completed",
            extra={
                "correlation_id": correlation_id,
        ...
        if status not in CONT_S2_SUCCESS_STATUSES:
            raise S2CanonicalContractCoreError(
                code="CONT_S2_VALIDATION_FAILED",
                message=f"Validacao de contrato CONT S2 falhou com status '{status}'",
                action=(
                    "Revisar logs de validacao estrita e conferir schema_required_fields/"
                    "domain_constraints do contrato canonico."
                ),
                correlation_id=correlation_id,
                stage="validation",
                event_id=completed_event_id,
        ...
        return S2CanonicalContractCoreOutput(
            contrato_versao=CONTRACT_VERSION,
            correlation_id=correlation_id,
            status="completed",
            contract_id=scaffold.contract_id,
            dataset_name=scaffold.dataset_name,
            validation_profile=dict(scaffold.validation_profile),
            execucao={
                "status": status,
                ...
                "schema_checks_executed": int(normalized.get("schema_checks_executed", 1)),
                "domain_checks_executed": int(normalized.get("domain_checks_executed", 1)),
            },
        ...
        logger.warning(
            "contract_validation_s2_main_flow_scaffold_error",
            extra={
                "correlation_id": correlation_id,
    ...
    except S2CanonicalContractCoreError:
        raise
    except Exception as exc:
        failed_event_id = _new_event_id()
        logger.error(
            "contract_validation_s2_main_flow_unexpected_error",
            extra={
                "correlation_id": correlation_id,
                "event_id": failed_event_id,
        ...
        raise S2CanonicalContractCoreError(
            code="CONT_S2_MAIN_FLOW_FAILED",
            message=f"Falha ao executar fluxo principal CONT S2: {type(exc).__name__}",
            action="Revisar logs operacionais e validar contrato de entrada/executor CONT S2.",
            correlation_id=correlation_id,
            stage="main_flow",
            event_id=failed_event_id,
        ) from exc
...
def _normalize_execution_response(
    *,
    ...
    if status not in CONT_S2_ALLOWED_EXEC_STATUSES:
        raise S2CanonicalContractCoreError(
...
def _default_validator(context: dict[str, Any]) -> dict[str, Any]:
    """Execute one strict schema/domain validation pass in dry-run mode.

    Args:
        context: Structured validation context with schema/domain metadata.
...
def _new_event_id() -> str:
    return f"conts2coreevt-{uuid4().hex[:12]}"
