"""MTC PDF metric extractor with gap-aware staging load.

This module extracts MTC press/clipping metric candidates from PDF text using
regex anchors and records explicit `gap` rows when evidence/value is missing.
"""
...
try:  # pragma: no cover
    ...
    from app.models.stg_dimac_mtc import MetricExtractionStatus, StgMtcMetric
...
_MTC_RULES: tuple[MetricRule, ...] = (
    MetricRule(
        metric_key="mtc.press.clippings_summary",
        metric_label="Resumo de clipping/imprensa",
        anchor_pattern=r"(clipping|imprensa|mat[eé]rias?|publica[cç][oõ]es|men[cç][oõ]es)",
        unit="count",
...
def extract_mtc_pdf_metrics(pdf_path: str | Path) -> Iterator[MetricCandidate]:
    """Extract MTC metric candidates from one PDF.

    Args:
        pdf_path: Source PDF path.

    Yields:
        `MetricCandidate` rows for MTC metrics.
    ...
    candidates, _ = extract_metric_candidates_from_pdf(pdf_path, _MTC_RULES)
    for candidate in candidates:
        yield candidate


def load_mtc_pdf_to_staging(
    *,
    source_id: str,
    pdf_path: str | Path,
    ...
    """Load MTC metric candidates into `stg_mtc_metrics`.
    ...
    with Session(engine) as session:
        ...
        run = start_ingestion_run(session, source.source_id, "extract_pdf_mtc_metrics")
        loaded_rows: list[StgMtcMetric] = []
        rows_requiring_lineage: list[StgMtcMetric] = []
        try:
            candidates, profile = extract_metric_candidates_from_pdf(pdf_path, _MTC_RULES)
            for candidate in candidates:
            ...
            if gap_count > 0:
                ...
                if normalized_gap_severity == "failed":
                    raise ValueError(
                        "Extracao MTC falhou por lacunas de evidencia/valor. "
            ...
            finish_ingestion_run(
                ...
                notes=(
                    f"pdf_profile={json.dumps(profile, ensure_ascii=False, sort_keys=True)}; "
                    f"{len(loaded_rows)} metricas carregadas em stg_mtc_metrics."
                ),
            )
            return {
                "source_id": source.source_id,
            ...
            if persisted_run is not None and persisted_run.finished_at is None:
                finish_ingestion_run(
                    ...
                    notes=f"Falha no load stg_mtc_metrics: {exc}",
                )
            raise


def main(argv: list[str] | None = None) -> int:
    """CLI entrypoint for MTC PDF extraction and staging load."""

    parser = argparse.ArgumentParser(prog="pdf-mtc")
    ...
    result = load_mtc_pdf_to_staging(
