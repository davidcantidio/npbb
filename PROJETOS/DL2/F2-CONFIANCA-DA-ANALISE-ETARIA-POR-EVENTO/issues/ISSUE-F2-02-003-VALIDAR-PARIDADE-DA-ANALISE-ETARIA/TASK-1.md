---
doc_id: "TASK-1.md"
issue_id: "ISSUE-F2-02-003-VALIDAR-PARIDADE-DA-ANALISE-ETARIA"
task_id: "T1"
version: "1.0"
status: "todo"
owner: "PM"
...
# T1 - Validar paridade da analise etaria

## objetivo

Ajustar fixtures e testes da analise etaria para provar paridade e nao regressao de contrato.
...
- testes_a_escrever_primeiro:
  - cobrir o contrato principal de `ISSUE-F2-02-003` no modulo alvo
  - cobrir um caso de regressao ligado ao paradigma canonico `LeadEvento`
- comando_para_rodar:
  - `cd backend && PYTHONPATH=.. SECRET_KEY=ci-secret-key TESTING=true python3 -m pytest -q tests/test_dashboard_age_analysis_endpoint.py`
- criterio_red:
...
- `cd backend && PYTHONPATH=.. SECRET_KEY=ci-secret-key TESTING=true python3 -m pytest -q tests/test_dashboard_age_analysis_endpoint.py`
- `rg -n "lead_evento|LeadEvento|LeadEventoSourceKind|AtivacaoLead|evento_nome|lead_batch|manual_reconciled|dashboard" backend/tests/test_dashboard_age_analysis_endpoint.py backend/tests/test_dashboard_age_analysis_service.py`
...
- fixtures e asserts da analise etaria refletem `LeadEvento`
- o contrato externo continua inalterado
