...
# ISSUE-F1-02-001 - Ajustar manifesto F2 ao split canonico de gate

## User Story

Como mantenedor do sibling `REVALIDACAO-F2-F3-GATE`, quero ajustar o manifesto
de F2 apenas no split canonico de checklist/gate para remover o drift
documental sem mexer em qualquer outra parte da fase.

## Contexto Tecnico

Esta issue depende da baseline `ISSUE-F1-01-001`. Se F2 for classificada como
`drift confirmado`, a execucao deve aplicar apenas o split
`pending -> hold` / `pending -> approved`. Se F2 estiver aderente, a issue deve
fechar como `no-op controlado`, com evidencia explicita e sem editar por editar.
Qualquer divergencia fora de checklist/gate e bloqueio de escopo.

## Plano TDD

- Red: confirmar a classificacao de F2 na baseline e isolar o trecho permitido de edicao
...
- Given a baseline classifica F2 como `drift confirmado`, When a issue for executada, Then o checklist de F2 passa a ter ramos separados `pending -> hold` e `pending -> approved`
- Given a baseline classifica F2 como `no-op controlado`, When a issue for executada, Then nenhum patch e aplicado e o resultado fica registrado explicitamente
...
### T1
- objetivo: usar a baseline para determinar exatamente o que esta issue pode tocar em F2
- precondicoes:
  - [Issue de Baseline](./ISSUE-F1-01-001-COMPARAR-MANIFESTOS-F2-E-F3-COM-O-TEMPLATE-CANONICO-DE-GATE.md) concluida
  - classificacao de F2 explicitada como `drift confirmado` ou `no-op controlado`
...
- passos_atomicos:
  1. localizar na baseline a classificacao final de F2
  2. identificar no manifesto F2 o bloco exato de checklist/gate que a issue pode tocar
  3. confirmar que nao existe necessidade de alterar nenhum campo fora desse bloco
- comandos_permitidos:
  - `sed`
  - `rg`
- resultado_esperado: faixa de edicao de F2 delimitada antes de qualquer patch
- testes_ou_validacoes_obrigatorias:
  - `rg -n "F2|drift confirmado|no-op controlado|BLOQUEADO" PROJETOS/FRAMEWORK2.0/REVALIDACAO-F2-F3-GATE/F1-REVALIDACAO-DOCUMENTAL-DE-GATE/issues/ISSUE-F1-01-001-COMPARAR-MANIFESTOS-F2-E-F3-COM-O-TEMPLATE-CANONICO-DE-GATE.md`
- stop_conditions:
  - parar se a baseline nao delimitar claramente o trecho de checklist/gate
  - parar se F2 exigir alteracao de `status`, `audit_gate`, `gate_atual` ou `ultima_auditoria`

### T2
- objetivo: aplicar apenas o patch minimo em F2 ou registrar no-op controlado
- precondicoes:
  - T1 concluida
  - faixa de edicao de F2 delimitada
- arquivos_a_ler_ou_tocar:
  - `PROJETOS/FRAMEWORK2.0/F2-ANTI-MONOLITH-ENFORCEMENT/F2_FRAMEWORK2_0_EPICS.md`
...
- passos_atomicos:
  1. se a baseline indicar `drift confirmado`, substituir em F2 a forma agregada `pending -> hold/approved` pelos blocos canonicos separados
...
- comandos_permitidos:
  ...
  - `rg`
  - `apply_patch`
- resultado_esperado: F2 corrigida apenas no checklist/gate ou issue encerrada como no-op controlado
- testes_ou_validacoes_obrigatorias:
  - `rg -n "pending -> hold|pending -> approved|pending -> hold/approved" PROJETOS/FRAMEWORK2.0/F2-ANTI-MONOLITH-ENFORCEMENT/F2_FRAMEWORK2_0_EPICS.md`
- stop_conditions:
...
### T3
- objetivo: comprovar que a issue nao introduziu alteracoes indevidas em F2
- precondicoes:
  - T2 concluida
...
- comandos_permitidos:
  ...
  - `rg`
  - `apply_patch`
- resultado_esperado: validacao final de que F2 permaneceu em estado pre-auditoria e sem drift adicional introduzido
- testes_ou_validacoes_obrigatorias:
  - `rg -n "^status:|^audit_gate:|gate_atual:|ultima_auditoria:|pending -> hold|pending -> approved" PROJETOS/FRAMEWORK2.0/F2-ANTI-MONOLITH-ENFORCEMENT/F2_FRAMEWORK2_0_EPICS.md`
- stop_conditions:
