"""Validation contracts for XIA Sprint 3 extraction on scanned documents.

This module centralizes XIA Sprint 3 input/output validation used by tests and
...
logger = logging.getLogger("npbb.etl.extract.ai.s3.validation")

XIA_S3_VALIDATION_VERSION = "xia.s3.validation.v1"
ALLOWED_FLOW_STATUSES = {"completed"}
ALLOWED_EXEC_STATUSES = {"succeeded", "completed", "success"}


class S3AIExtractValidationError(ValueError):
    """Raised when XIA Sprint 3 validation contract is violated."""
...
class S3AIExtractValidationInput:
    """Input contract consumed by XIA Sprint 3 validation checks."""
    ...
    def to_core_input(self, *, correlation_id: str | None = None) -> S3AIExtractCoreInput:
        """Convert validated data to `S3AIExtractCoreInput` contract."""
    ...
    def to_scaffold_request(
        ...
        *,
        ...
        """Convert validated data to `S3AIExtractScaffoldRequest` contract."""

        return S3AIExtractScaffoldRequest(
            source_id=self.source_id,
            source_kind=self.source_kind,
            source_uri=self.source_uri,
            document_profile_hint=self.document_profile_hint,
...
@dataclass(frozen=True, slots=True)
class S3AIExtractValidationResult:
    """Output contract returned by XIA Sprint 3 input validation."""
...
@dataclass(frozen=True, slots=True)
class S3AIExtractFlowOutputValidationResult:
    """Output contract returned by XIA Sprint 3 flow-output validation."""
...
def validate_s3_extract_ai_input_contract(
    payload: S3AIExtractValidationInput,
) -> S3AIExtractValidationResult:
    """Validate XIA Sprint 3 input contract before running the main flow.

    Args:
        payload: Input contract with source metadata and AI extraction params.

    Returns:
        S3AIExtractValidationResult: Validation metadata and checks summary.

    Raises:
        S3AIExtractValidationError: If mandatory contract rules are violated.
    """

    correlation_id = payload.correlation_id or f"xia-s3-val-{uuid4().hex[:12]}"
    started_event = _emit_observability_event(
        event_name="etl_extract_ai_s3_validation_started",
        correlation_id=correlation_id,
        event_message="Validacao de entrada do XIA S3 iniciada",
        severity="info",
        ...
        image_preprocess_hint=payload.image_preprocess_hint,
    ...
    completed_event = _emit_observability_event(
        event_name="etl_extract_ai_s3_validation_completed",
        correlation_id=correlation_id,
        event_message="Validacao de entrada do XIA S3 concluida com sucesso",
        severity="info",
        ...
        image_preprocess_hint=scaffold.extraction_plan.get("image_preprocess_hint"),
        stage="validation_input",
    ...
    logger.info(
        "etl_extract_ai_s3_validation_result",
        extra={
            "validation_id": validation_id,
            "correlation_id": correlation_id,
            "status": "valid",
            "checks": checks,
            "route_preview": "ai_documento_scan_s3",
        },
    )
    return S3AIExtractValidationResult(
        validation_version=XIA_S3_VALIDATION_VERSION,
        validation_id=validation_id,
        correlation_id=correlation_id,
        status="valid",
        checks=tuple(checks),
...
def validate_s3_extract_ai_flow_output_contract(
    flow_output: dict[str, Any],
    *,
    correlation_id: str,
) -> S3AIExtractFlowOutputValidationResult:
    """Validate XIA Sprint 3 flow output contract.

    Args:
        flow_output: Output dictionary produced by XIA S3 core or service flow.
        correlation_id: Correlation identifier propagated by caller.

    Returns:
        S3AIExtractFlowOutputValidationResult: Summary of checked fields.

    Raises:
        S3AIExtractValidationError: If output contract is incomplete or
            inconsistent with XIA Sprint 3 specifications.
    """
    ...
    if missing_fields:
        _raise_validation_error(
            ...
            message=f"Saida do fluxo XIA S3 incompleta: faltam campos {missing_fields}",
    ...
    if status not in ALLOWED_FLOW_STATUSES:
        _raise_validation_error(
            ...
            message=f"Status final inesperado no fluxo XIA S3: {flow_output.get('status')}",
            action="Garanta status 'completed' para saidas validas do XIA S3.",
            correlation_id=correlation_id,
            stage="validation_output",
    ...
    logger.info(
        "etl_extract_ai_s3_flow_output_validated",
        extra={
            "correlation_id": correlation_id,
            "status": "valid",
            "layer": layer,
    ...
    return S3AIExtractFlowOutputValidationResult(
        correlation_id=correlation_id,
        status="valid",
        layer=layer,
        checked_fields=required_fields,
...
def _resolve_output_layer(
    *,
    ...
    if contract_version == "xia.s3.core.v1":
        return (
            "core",
            {
                "flow_started_event_id": "xias3coreevt-",
                "flow_completed_event_id": "xias3coreevt-",
            },
        )
    if contract_version == "xia.s3.service.v1":
        return (
            "service",
            {
                "flow_started_event_id": "xias3evt-",
                "plan_ready_event_id": "xias3evt-",
                "flow_completed_event_id": "xias3evt-",
                "main_flow_started_event_id": "xias3coreevt-",
                "main_flow_completed_event_id": "xias3coreevt-",
            },
        )
    raise S3AIExtractValidationError(
        code="UNSUPPORTED_OUTPUT_CONTRACT_VERSION",
        message=f"versao de contrato nao suportada para validacao: {contract_version}",
        action="Use contratos xia.s3.core.v1 ou xia.s3.service.v1.",
        correlation_id=correlation_id,
        stage="validation_output",
    )
...
def _raise_validation_error(
    *,
    ...
    failed_event = _emit_observability_event(
        event_name="etl_extract_ai_s3_validation_failed",
        correlation_id=correlation_id,
    ...
    logger.warning(
        "etl_extract_ai_s3_validation_error",
        extra={
            "correlation_id": correlation_id,
            "error_code": code,
...
def _emit_observability_event(
    *,
    ...
    log_s3_extract_ai_observability_event(event)
    return event
