---
doc_id: "TASK-1.md"
issue_id: "ISSUE-F3-01-002-Implementar-geracao-de-PRD-com-rastreabilidade-do-gate"
task_id: "T1"
version: "1.1"
status: "todo"
owner: "PM"
last_updated: "2026-03-17"
tdd_aplicavel: true
---

# TASK-1 - Implementar geracao de PRD a partir do intake aprovado

## objetivo

gerar PRD vinculado ao intake e persistir o artefato no backend

## precondicoes

- `ISSUE-F3-01-001` concluida

## arquivos_a_ler_ou_tocar

- `backend/app/services/framework_orchestrator.py`
- `backend/app/api/v1/endpoints/framework.py`
- `backend/tests/test_framework_prd_generation.py`

## testes_red

- testes_a_escrever_primeiro:
  - cobrir geracao de PRD vinculada ao intake e persistencia do artefato
- comando_para_rodar:
  - `cd backend && SECRET_KEY=ci-secret-key TESTING=true .venv/bin/python -m pytest -q tests/test_framework_prd_generation.py`
- criterio_red:
  - os testes acima devem falhar antes da implementacao; se passarem, parar e revisar a task

## passos_atomicos

1. escrever os testes listados em `testes_red`
2. rodar os comandos red e confirmar falha inicial ligada ao contrato esperado
3. implementar o minimo necessario para fazer apenas os cenarios desta task passarem
4. rodar novamente as suites alvo e confirmar green
5. refatorar nomes contratos ou extracoes locais sem ampliar o escopo mantendo a suite green

## comandos_permitidos

- `cd backend && SECRET_KEY=ci-secret-key TESTING=true .venv/bin/python -m pytest -q tests/test_framework_prd_generation.py`

## resultado_esperado

PRD gerado a partir do intake aprovado.

## testes_ou_validacoes_obrigatorias

- `cd backend && SECRET_KEY=ci-secret-key TESTING=true .venv/bin/python -m pytest -q tests/test_framework_prd_generation.py`

## stop_conditions

- parar se a geracao exigir uma decisao aberta de template
