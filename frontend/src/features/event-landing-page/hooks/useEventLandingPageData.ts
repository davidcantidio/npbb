...
import { DUPLICATE_CPF_BLOCKED_MESSAGE } from "../constants";

export type FormState = Record<string, string>;

export type UseEventLandingPageDataParams = {
  resolvedEventId: number | null;
  resolvedAtivacaoId: number | null;
...
export function useEventLandingPageData({
  resolvedEventId,
  resolvedAtivacaoId,
  landingToken,
}: UseEventLandingPageDataParams) {
  ...
  const handleInputChange = useCallback(
    ...
    [data, selectedVariant?.id, sessionId],
  );

  const handleCpfFirstContinue = useCallback(() => {
  ...
  }, [cpfFirstEnabled, formState.cpf]);

  const handleRegistrarOutroCpf = useCallback(() => {
  ...
  const handleGamificacaoComplete = useCallback(async (gamificacaoId: number) => {
    if (!ativacaoLeadId) {
  ...
  return {
    ...
    cpfFirstEnabled,
    cpfFirstUnlocked,
    showRegistrarOutroCpfPrompt,
    leadSubmitted,
    ativacaoLeadId,
    gamificacaoBusy,
    gamificacaoBlockedReason,
    gamificacaoResetVersion,
    handleInputChange,
    handleCpfFirstContinue,
    handleRegistrarOutroCpf,
    handleSubmit,
    handleGamificacaoComplete,
    handleReset,
  };
}
