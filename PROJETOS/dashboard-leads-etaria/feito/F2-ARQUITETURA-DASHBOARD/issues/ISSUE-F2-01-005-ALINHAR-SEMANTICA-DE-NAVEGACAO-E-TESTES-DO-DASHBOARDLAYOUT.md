...
- [x] Sidebar do dashboard expõe landmark de navegacao com nome acessivel estavel.
- [x] Cenario com `app-sidebar-slot` e cenario fallback inline passam no `DashboardLayout.test.tsx`.
- [x] Comportamento mobile continua usando slot de sidebar do `AppLayout` sem duplicacao.
- [x] Nao ha regressao em `DashboardHome.test.tsx` e `DashboardModule.test.tsx`.

## Definition of Done da Issue

- [x] Criterios de aceitacao validados por testes locais
- [x] Ajuste documentado no arquivo da issue e vinculado ao `RELATORIO-AUDITORIA-F2-R02`
- [x] Nenhum desvio adicional de escopo introduzido na fase F2

## Tarefas Decupadas

- [x] T1: Revisar o markup atual de `DashboardSidebar`/`DashboardLayout` para definir o landmark de navegacao.
- [x] T2: Ajustar componente para expor semantica acessivel consistente nos modos portal e fallback.
- [x] T3: Atualizar/estabilizar assercoes de teste se houver drift nominal justificado.
- [x] T4: Reexecutar pacote de testes F2 (`DashboardLayout`, `DashboardHome`, `DashboardModule`).
...
- comando: `cd /Users/genivalfreirenobrejunior/Documents/code/npbb/npbb/frontend && npm run test -- --run src/components/dashboard/__tests__/DashboardLayout.test.tsx src/pages/dashboard/__tests__/DashboardHome.test.tsx src/pages/dashboard/__tests__/DashboardModule.test.tsx`
- resultado: `3 arquivos, 10 testes passando`
