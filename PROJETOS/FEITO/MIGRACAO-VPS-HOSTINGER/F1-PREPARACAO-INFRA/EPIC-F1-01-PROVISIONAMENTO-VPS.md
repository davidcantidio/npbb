...
- `docs/DEPLOY_HOSTINGER_VPS.md` trata o script acima como caminho oficial de bootstrap.
...
**Plano TDD**
1. `Red`: ampliar `docs/DEPLOY_HOSTINGER_VPS.md` e `scripts/check_production_stack_contracts.sh` para falhar quando faltarem pacotes, diretorios base ou o usuario `deploy` no fluxo documentado.
2. `Green`: criar `Infra/production/scripts/init-vps.sh` para instalar `ca-certificates`, `curl`, `git`, `ufw`, `fail2ban`, Docker Engine e Docker Compose plugin, criar `deploy`, adiciona-lo ao grupo `docker` e provisionar diretorios em `/opt/npbb`.
...
- Given drift entre script e documentacao, When `./scripts/check_production_stack_contracts.sh` roda, Then a validacao falha ate que `docs/DEPLOY_HOSTINGER_VPS.md` reflita o bootstrap real.
...
**Plano TDD**
1. `Red`: endurecer `scripts/deploy_vps.sh` e `scripts/check_production_stack_contracts.sh` para falhar quando uma variavel usada por deploy ou smoke nao estiver declarada em `Infra/production/.env.example`.
...
**Plano TDD**
1. `Red`: fazer `scripts/check_production_stack_contracts.sh` falhar quando nao houver `ufw`, `fail2ban`, instrucao de chave SSH e proibicao do fluxo root como caminho normal de operacao.
2. `Green`: incorporar no `Infra/production/scripts/init-vps.sh` a configuracao de `ufw` para `22/80/443`, habilitacao de `fail2ban` e instrucoes objetivas de `authorized_keys` e endurecimento pos-bootstrap em `docs/DEPLOY_HOSTINGER_VPS.md`.
