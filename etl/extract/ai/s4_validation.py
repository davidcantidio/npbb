"""Validation contracts for XIA Sprint 4 calibrated extraction.

This module centralizes XIA Sprint 4 input/output validation used by tests and
...
logger = logging.getLogger("npbb.etl.extract.ai.s4.validation")

XIA_S4_VALIDATION_VERSION = "xia.s4.validation.v1"
ALLOWED_FLOW_STATUSES = {"completed"}
ALLOWED_EXEC_STATUSES = {"succeeded", "completed", "success"}


class S4AIExtractValidationError(ValueError):
    """Raised when XIA Sprint 4 validation contract is violated."""
...
class S4AIExtractValidationInput:
    """Input contract consumed by XIA Sprint 4 validation checks."""
    ...
    def to_core_input(self, *, correlation_id: str | None = None) -> S4AIExtractCoreInput:
        """Convert validated data to `S4AIExtractCoreInput` contract."""
    ...
    def to_scaffold_request(
        ...
        *,
        ...
        """Convert validated data to `S4AIExtractScaffoldRequest` contract."""
...
@dataclass(frozen=True, slots=True)
class S4AIExtractValidationResult:
    """Output contract returned by XIA Sprint 4 input validation."""
...
@dataclass(frozen=True, slots=True)
class S4AIExtractFlowOutputValidationResult:
    """Output contract returned by XIA Sprint 4 flow-output validation."""
...
def validate_s4_extract_ai_input_contract(
    payload: S4AIExtractValidationInput,
) -> S4AIExtractValidationResult:
    """Validate XIA Sprint 4 input contract before running the main flow.

    Args:
        payload: Input contract with source metadata and AI extraction params.

    Returns:
        S4AIExtractValidationResult: Validation metadata and checks summary.

    Raises:
        S4AIExtractValidationError: If mandatory contract rules are violated.
    """

    correlation_id = payload.correlation_id or f"xia-s4-val-{uuid4().hex[:12]}"
    started_event = _emit_observability_event(
        event_name="etl_extract_ai_s4_validation_started",
        correlation_id=correlation_id,
        event_message="Validacao de entrada do XIA S4 iniciada",
        severity="info",
        ...
        quality_profile_hint=payload.quality_profile_hint,
        consolidation_scope=payload.consolidation_scope,
        output_normalization_profile=payload.output_normalization_profile,
    ...
    completed_event = _emit_observability_event(
        event_name="etl_extract_ai_s4_validation_completed",
        correlation_id=correlation_id,
        event_message="Validacao de entrada do XIA S4 concluida com sucesso",
        severity="info",
        ...
        output_normalization_profile=scaffold.extraction_plan.get("output_normalization_profile"),
        stage="validation_input",
    ...
    logger.info(
        "etl_extract_ai_s4_validation_result",
        extra={
            "validation_id": validation_id,
            "correlation_id": correlation_id,
            "status": "valid",
            "checks": checks,
            "route_preview": "ai_calibrada_s4",
        },
    )
    return S4AIExtractValidationResult(
        validation_version=XIA_S4_VALIDATION_VERSION,
        validation_id=validation_id,
        correlation_id=correlation_id,
        status="valid",
        checks=tuple(checks),
...
def validate_s4_extract_ai_flow_output_contract(
    flow_output: dict[str, Any],
    *,
    correlation_id: str,
) -> S4AIExtractFlowOutputValidationResult:
    """Validate XIA Sprint 4 flow output contract.

    Args:
        flow_output: Output dictionary produced by XIA S4 core or service flow.
        correlation_id: Correlation identifier propagated by caller.

    Returns:
        S4AIExtractFlowOutputValidationResult: Summary of checked fields.

    Raises:
        S4AIExtractValidationError: If output contract is incomplete or
            inconsistent with XIA Sprint 4 specifications.
    """
    ...
    if missing_fields:
        _raise_validation_error(
            ...
            message=f"Saida do fluxo XIA S4 incompleta: faltam campos {missing_fields}",
    ...
    if status not in ALLOWED_FLOW_STATUSES:
        _raise_validation_error(
            ...
            message=f"Status final inesperado no fluxo XIA S4: {flow_output.get('status')}",
            action="Garanta status 'completed' para saidas validas do XIA S4.",
            correlation_id=correlation_id,
            stage="validation_output",
    ...
    logger.info(
        "etl_extract_ai_s4_flow_output_validated",
        extra={
            "correlation_id": correlation_id,
            "status": "valid",
            "layer": layer,
    ...
    return S4AIExtractFlowOutputValidationResult(
        correlation_id=correlation_id,
        status="valid",
        layer=layer,
        checked_fields=required_fields,
...
def _resolve_output_layer(
    *,
    ...
    if contract_version == "xia.s4.core.v1":
        return (
            "core",
            {
                "flow_started_event_id": "xias4coreevt-",
                "flow_completed_event_id": "xias4coreevt-",
            },
        )
    if contract_version == "xia.s4.service.v1":
        return (
            "service",
            {
                "flow_started_event_id": "xias4evt-",
                "plan_ready_event_id": "xias4evt-",
                "flow_completed_event_id": "xias4evt-",
                "main_flow_started_event_id": "xias4coreevt-",
                "main_flow_completed_event_id": "xias4coreevt-",
            },
        )
    raise S4AIExtractValidationError(
        code="UNSUPPORTED_OUTPUT_CONTRACT_VERSION",
        message=f"versao de contrato nao suportada para validacao: {contract_version}",
        action="Use contratos xia.s4.core.v1 ou xia.s4.service.v1.",
        correlation_id=correlation_id,
        stage="validation_output",
    )
...
def _raise_validation_error(
    *,
    ...
    failed_event = _emit_observability_event(
        event_name="etl_extract_ai_s4_validation_failed",
        correlation_id=correlation_id,
    ...
    logger.warning(
        "etl_extract_ai_s4_validation_error",
        extra={
            "correlation_id": correlation_id,
            "error_code": code,
...
def _emit_observability_event(
    *,
    ...
    log_s4_extract_ai_observability_event(event)
    return event
