...
def _create_enums_if_postgres() -> None:
    ...
    if bind.dialect.name != "postgresql":
        return
    # Usa DO...EXCEPTION em vez de checkfirst=True para garantir idempotência mesmo
    # quando o tipo foi criado fora do Alembic (ex: metadata.create_all na app).
    op.execute(
        sa.text(
            "DO $$ BEGIN CREATE TYPE tipoingresso AS ENUM "
    ...
    op.execute(
        sa.text(
            "DO $$ BEGIN CREATE TYPE modofornecimento AS ENUM "
...
def upgrade() -> None:
    ...
    if not insp.has_table("configuracao_ingresso_evento"):
        op.create_table(
    ...
    if not insp.has_table("configuracao_ingresso_evento_tipo"):
        op.create_table(
    ...
    if not insp.has_table("previsao_ingresso"):
        op.create_table(
            "previsao_ingresso",
            sa.Column("id", sa.Integer(), nullable=False),
            sa.Column("evento_id", sa.Integer(), nullable=False),
